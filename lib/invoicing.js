...
let listPayOptions = bluebird.promisify((user, done) => {
...
let createInvoice = bluebird.promisify((user, options, done) => {
  let merchantKey = user.merchantKey,
    emailOrMobileNumber = user.emailOrMobileNumber;
  if (!options.amount)
    return done({
  ...
  if (options.sendInvoice) {
    ...
    if (!options.customerEmail && !options.customerMobileNumber)
      ...
      if (options.customerMobileNumber)
        payload.customerMobileNumber = options.customerMobileNumber;
    }
  }
...
let sendInvoice = bluebird.promisify((user, options, done) => {
  let merchantKey = user.merchantKey,
    emailOrMobileNumber = user.emailOrMobileNumber;
  let payload = {
    merchantKey: merchantKey,
  ...
  else if (options.customerMobileNumber)
    payload.customerMobileNumber = options.customerMobileNumber;
...
let checkPaymentStatus = bluebird.promisify((user, options, done) => {
  let merchantKey = user.merchantKey,
    emailOrMobileNumber = user.emailOrMobileNumber;
  let payload = {
    merchantKey: merchantKey,
